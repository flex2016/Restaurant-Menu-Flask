...
# store values in it for the longgevity of the user in session used for showlogin
...
app = Flask(__name__)
# CLIENT_ID is referenced to the json file for google sign in
...
def upload(restaurant_id, menu_id):
    restaurant = session.query(Restaurant).filter_by(id=restaurant_id).one()
    item = session.query(MenuItem).filter_by(id=menu_id).one()  # find item to edit
...
def slashboard():
    try:
        return render_template('publicrestaurants.html', item=item)
